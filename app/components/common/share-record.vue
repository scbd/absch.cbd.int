...
<script setup>
  ...
  const auth = useAuth();
  const {getQuery, userStatus, generateLink, isUserSignedIn} = defineProps({
    getQuery: Function,
    userStatus: Function,
  ...
  onMounted(async () => { 
    ...
    modal = new Modal(shareModal.value); // new Modal(ref('shareModal')); // ???
    toast = new Toast(document.getElementById('clipboardToast'));
  });

  const openModel = async () => {  
    if (!auth.token()) 
        userToken = await auth.token();
    if (!auth.token() && !isUserSignedIn)
        isUserSignedIn = true;

    if (!modal._isShown)
        modal.show('static');

    if (sharedData.value.type != 'link' && !userToken)
        userStatus;
    else
...
const loadTabData = async (type) => {
  ...
  const { recordKey, type: storageType, query } = getQuery; //TODO: change, GetQuery is getting details about what type of share it is (id and type search-relesult/document)
  ...
  if (sharedData.value.storageType == "chm-document") {
    return sharedData.value[type].link = `${realm.baseURL}/${$locale}/database/${sharedData.value[type].recordKey}`;
  }

  if (!userToken && sharedData.value.type != 'link' &&
  ...
  if (sharedData.value.storageType == "chm-country-profile") {
    sharedData.value[type].link = `${realm.baseURL}/${$locale}/countries/${sharedData.value[type].recordKey}`;
  }
  refreshSharedData();
}
...
const shareLinkMail = async () => {
  ...
  if (emails.split(/,|;| /g).length > 10) {
    error.value = "i18n.excessEmailError";
    return;
  }
  try {
    console.log("shareLinkMail");
    loading.value = true;
    sharedData.value[sharedData.value.type].emails = [...(new Set(emails.replace(/(,|;|\s)$/, '').split(/,|;| /g)))].filter(e => e).join(',');
    console.log("getRecaptchaToken 111111",getRecaptchaToken);
    ...
    sharedData.value[sharedData.type].link = `${realm.baseURL}/${$locale}/share/link/${sharedData.value.storageType}/${sharedData.value[sharedData.value.type].urlHash}`;
...
const generateEmbedCode = async () => {
  ...
  try {
    ...
    if (!sharedData.value.embed._id) {
      console.log("cccccccccccccc",sharedData.value);
      if (sharedData.value.storageType == "chm-search-result") {
        console.log("inside ",sharedData.value);
        await saveSearchQuery();
      }
    ...
    console.log(err)
    error.value = "i18n.embedCodeError";
  } finally {
    loading.value = false;
  }
...
const saveShareDocument = async (data, captchaToken) => {
  ...
  if (sharedData.value[sharedData.value.type].recordKey) {
    data.sharedData.value.recordKey = sharedData.value[sharedData.value.type].recordKey;
    ...
    console.log(`i18n captchaToken captchaToken`,captchaToken);
...
const saveSearchQuery = async () => {
  ...
  console.log(`saveSearchQuery search`, data)
...
const generateSearchResultLink = async () => {
  console.log("Generating", sharedData.value.type);
  console.log("Generating val", sharedData.value.storageType);
  error.value = undefined;
  loading.value = true;
  try {
    if (!sharedData.value[sharedData.value.type]._id) {
      console.log(" cleaned err xxxxxx");
      const captchaToken = await getRecaptchaToken;
      
      console.log(" cleaned err xxxxxx", captchaToken);
      const data = {}
      if (sharedData.value.storageType == "chm-search-result") {
    ...
    sharedData.value[sharedData.value.type].link = `${realm.baseURL}/${$locale}/share/link/${sharedData.value.storageType}/${sharedData.value[sharedData.value.type].urlHash}`;
    refreshSharedData();
  }
  catch (err) {
    console.log(" cleaned err",err);
    console.error(err);
    if (err?.code == "INVALID_CAPTCHA_SCORE")
    error.value = err.message;
    else
    error.value = "i18n.searchResultLinkError";
  }
  finally {
    console.log(" cleaned up",false);
    loading.value = false;
  }
};

const onDomainChange = () => {
  console.log(" domain",  sharedData);
  const regexp =
...
const getShareDocumentData = () => {
  console.log("embed sharedData.value getShareDocumentData");
  console.log("embed sharedData.value realm", realm);
  const data = {
...
console.log("data",data);
  if (['embed', 'link'].includes(sharedData.value.type)) {
    ...
    console.log("embed sharedData.value",sharedData.value);
    if (sharedData.value.type == 'embed')
      data.sharedData.value.domain = sharedData.value[sharedData.value.type].domain;
  }
...
const getSearchQueryData = () => {
  console.log("bbbbbbbbbbbbbb",sharedData.value)
  console.log("bbbbbbbbbbbbbb",sharedData.value[sharedData.value.type])
  console.log("bbbbbbbbbbbbbb",sharedData.value[sharedData.value.type].searchQuery)
  const { filters, subFilters } = sharedData.value[sharedData.value.type].searchQuery || {};
  console.log(`filters, subFilters search`, filters, subFilters)
  const data = {
...
const refreshSharedData = () => {
  console.log("Refreshing shared data", sharedData);
  console.log("Refreshing shared data 11111", sharedData.value);
