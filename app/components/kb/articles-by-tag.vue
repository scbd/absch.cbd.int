...
<script setup>
    ...
    onMounted(async () => {  
        tag.value = (route.value?.params?.tag).replace(/"/g, ""); 
        if (tag.value != undefined && tag.value != null) {
            categories.value = await loadKbCategories(realm.is('BCH'));
            tagDetails = categories.value.find(e => e.adminTags.includes(tag.value)) || {
                title: tag.value
            };
            tag.value = tag.value; //ToDo: ???
            loadArticles(1, tag.value);
        }
    })
