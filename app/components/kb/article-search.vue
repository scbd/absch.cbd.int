...
// if search changed
//   watch(() => props.route.params.search, (newSearch) => {
//     if (newSearch) {
//       search.value = decodeURIComponent(newSearch).replace(/"/g, "");
//     }
//   });
    ...
    const loadArticles = async (page) => {
              ...
              if (search.value !== undefined) {
                // console.log("2222222",$locale.value);
                // console.log("3333",this.locale)
              ...
              searchAg.push({
                  "$limit": pageNumber * recordsPerPage.value
              });
              searchAg.push({
                  "$skip": (pageNumber - 1) * recordsPerPage.value
              });
              countAg.push({
                  "$count": "count"
              });
    ...
    onMounted( async() => {
      ...
      categories.value = loadKbCategories(realm.is('BCH'));
      if (route.value?.params?.search) {
          search.value = decodeURIComponent(route.value.params.search).replace(/"/g, "");
      }
