<template>
  <div class="flex-grow-1 d-flex flex-column align-items-center justify-content-center gap-2 p-5">
    ...
    <div class="fw-bold" style="font-size: 15px; color: #1a2e3b;">
      {{ fileName }}
    </div>
    ...
    <ul class="list-unstyled m-0 mt-1 p-0 d-flex flex-column gap-2" style="width: min(440px, 90%);">
      <li
        v-for="step in steps" :key="step.key"
        class="bi-step d-flex align-items-center gap-2 small" :class="`bi-step--${step.status}`"
      >
        <span
