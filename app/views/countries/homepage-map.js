import template from 'text!./homepage-map.html';
import app from 'app';
...
import './directives/homepin-popup-abs';
  ...
  app.directive('homepageMap', ['$timeout', function($timeout) {
    return {
      restrict: 'EAC',
      template: template,
      replace: true,
      ...
      function($scope, commonjs, searchService, $q, $interpolate, $filter, $location, $element, localeService, locale, realm, $http, $routeParams, solr) {
        ...
        function countryFacets(code){
          return $q.all([searchService.governmentSchemaFacets(code),
                loadRegionalRecordFacets(code)])
                  .then(function(result) { 
                    const countryFacets = result[0]||{};
                    const regionalFacets = result[1]||{};
                    countryFacets.schemas = (countryFacets||{}).schemas||{};
                    
                    if(code)                     
                        return normalizeFacets(countryFacets.schemas, {}, regionalFacets);

                    //loop array for global facets
                    var facets = {};
                    _.forEach(countryFacets, function(countrySchema){
                        normalizeFacets(countrySchema.schemas, facets, regionalFacets)
                    })
                    return facets;
                  });
        }

        function normalizeFacets(countrySchemas, facets, regionalFacets){
          if(!facets)
            facets = {};
          ...
          _.forEach(realm.schemas, function(schema, key){
            if(schema.type=='national' && key!=='contact'){
              ...
              //special loop to insert regional facets for eg. show EU Laws in all EU member states profile; 
              if(regionalFacets && regionalFacets.recordCount>0){
                    if(regionalFacets.schemas[key]!=undefined){
                      facets[`${schema.shortCode}_${regionalFacets.government}`] = {
                          ...facets[schema.shortCode],
                          count             : regionalFacets.schemas[key],
                          isRegional        : true,
                          regionalGovernment:{identifier:regionalFacets.government}
                        }
                    }
              }
