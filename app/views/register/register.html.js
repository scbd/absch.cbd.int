define(['app', 'linqjs', 'angular-localizer',
    ...
    app.controller("RegisterController", ["$rootScope", "$location", "$scope", "$q", "$window", "IStorage", "underscore", "breadcrumbs",
        ...
        function($rootScope, $location, $scope, $q, $window, storage, _, breadcrumbs,
            ...
            if ($routeParams.document_type) { //this is used to highlight the item on the left
                ...
                breadcrumbs.options = {
                    'document_type': $filter("schemaName")($scope.document_type)
                };
            }
            $scope.msg = "";
            $scope.records = [];
                ...
                $q.all([published, drafts, requests]).then(function(results) {
                    ...
                    _.each(results, function(facets) {
                        _.each(facets.data, function(count, format) {
                            ...
                            var schemaTypeFacet = _.where($scope.schemaTypesFacets, {
                                "schema": format
                            });
                    ...
                    if ($rootScope.next_url) {
                        ...
                        $timeout(function() {
                            $location.path('/register/' +
                                $filter("mapSchema")($scope.document_type));
                        }, 100);
                    }
                }, 500);
            ...
            $scope.$on("documentPublishRequested", function(evt, workflowInfo) {
                ...
                $timeout(function() {
                    $location.path('/register/' + $filter("mapSchema")($scope.document_type));
                }, 100);
