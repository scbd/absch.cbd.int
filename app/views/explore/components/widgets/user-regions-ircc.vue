  ...
  <div class="card h-100">
    <div class="card-body">
      <h5 class="card-title text-center">
        CBD regions where users are located
      </h5>
      ...
      >
        Unable to load user-region counts.
      </div>
...
interface UserRegionDefinition {
  id: string
  label: string
  fullLabel: string
  colour: string
...
interface UserRegionCount
  extends UserRegionDefinition {
  count: number
}

const USER_REGION_FILTER_ID =
  'user-region'

const USER_REGION_FIELD =
...
function buildRegionQuery (
  ...
  return `${USER_REGION_FIELD}:${regionId}`
}

function selectRegion (
  regionItem: UserRegionCount
): void {
  const {
    id,
    label
