module.name    = "absEditAbsCheckpointCommunique";
module.exports = ['authHttp', "URI", "guid", "$filter", function ($http, URI, guid, $filter) {
	return {
		restrict   : 'EAC',
		templateUrl: '/app/abs/directives/forms/form-abs-checkpoint-communique.partial.html',
		replace    : true,
		transclude : false,
		scope      : {},
		...
		controller: ['$scope', "$q", 'IStorage', "authentication", "Enumerable", "underscore", "editFormUtility", function ($scope, $q, storage, authentication, Enumerable, _, editFormUtility)
		{
			...
			$scope.init = function() {
				...
				if(identifier)
					promise = editFormUtility.load(identifier, "absCheckpointCommunique");
				else
					promise = $q.when({
						header: {
							identifier	: guid(),
