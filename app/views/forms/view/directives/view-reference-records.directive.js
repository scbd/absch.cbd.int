...
const REFERENCE_RECORD_KEY  = 'showAllReferenceRecordsKey';
						...
						_.forEach(record.referenceRecord_info_ss, function(info){
							...
							_.uniq(info.identifiers).forEach((identifier)=>{
								if(removeRevisionNumber(identifier) == $scope.model){
									...
									const storedValue = localStorage.getItem(REFERENCE_RECORD_KEY);
 
									const effectiveRowsPerPage = storedValue
									? parseInt(storedValue, 10) : 25;
									fieldInfo = {
										...fieldInfo,
