...
app.directive("viewReferencedRecords", [function () {
	return {
		...
		controller: ["$scope", "solr", 'searchService', 'realm', 'translationService', function ($scope, solr, searchService, realm, translationService) {
			...
			async function loadReferencedRecords(){
				$scope.loading = true;
				try{
					const { docs } = await loadSolrRecords();
			...
			async function loadSolrRecords(){
				const searchQuery = {
					rowsPerPage:2000,
				...
				const { docs, numFound } = result.data.response;
