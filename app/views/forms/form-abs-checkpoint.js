module.name    = "absEditAbsCheckpoint";
module.exports = ['authHttp', "URI", "guid", "$filter", function ($http, URI, guid, $filter) {
	return {
		restrict   : 'EAC',
		templateUrl: '/app/abs/directives/forms/form-abs-checkpoint.partial.html',
		replace    : true,
		transclude : false,
		scope      : {},
		...
		controller : ['$scope', "$q", 'IStorage', "authentication", "underscore", "editFormUtility", function ($scope, $q, storage, authentication, _, editFormUtility) 
		{
			//==================================
			//
			//==================================
			...
			$scope.init = function() {
				...
				if(identifier)
					promise = editFormUtility.load(identifier, "absCheckpoint");
				else
					promise = $q.when({
						header: {
							identifier: guid(),
