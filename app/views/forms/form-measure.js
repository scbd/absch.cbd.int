...
//
//============================================================
$compile.directive('cbdEditMeasure', ['authHttp', "URI", "Enumerable", "$filter", "$q", "guid", function ($http, URI, Enumerable, $filter, $q, guid) {
	return {
		restrict   : 'EAC',
		templateUrl: '/app/abs/directives/forms/form-measure.partial.html',
		replace    : true,
		transclude : false,
		scope      : {},
		link : function($scope, $element)
		{
			...
			$scope.options  = {
				...
				documentLinksExt :        [{ model:'language',        title:'Language',         required:true, options: $http.get("/api/v2013/thesaurus/domains/ISO639-2/terms", { cache: true }).then(function(o){ return $filter('orderBy')(o.data, 'name'); }) }],
				documentTranslationsExt : [{ model:'language',        title:'Language',         required:true, options: $http.get("/api/v2013/thesaurus/domains/ISO639-2/terms", { cache: true }).then(function(o){ return $filter('orderBy')(o.data, 'name'); }) },
										   { model:'translationType', title:'Translation Type', required:true, options: $http.get("/api/v2013/thesaurus/domains/19E3C535-2919-4804-966C-E62728507291/terms", { cache: true }).then(function(o){ return $filter('orderBy')(o.data, 'name'); }) },]
			};
			//UI Effects binding//
		...
		controller : ['$scope', "$q", 'IStorage', "authentication", "editFormUtility", function ($scope, $q, storage, authentication, editFormUtility) 
		{
			...
			$scope.init = function() {
				...
				if(identifier)
					promise = editFormUtility.load(identifier, "measure");
				else
					promise = $q.when({
						header: {
							identifier: guid(),
	...
	//
	//============================================================
	.directive('cbdEditMeasureAbs', function () {
		return {
			restrict: 'EAC',
			templateUrl: '/app/partials/cbd/edit/measure.html/abs',
			replace: true,
			transclude: false,
			require : "?ngModel",
			scope: {
