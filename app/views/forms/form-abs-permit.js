module.name    = "absEditAbsPermit";
module.exports = ['authHttp', "URI", "Enumerable", "guid", "$filter", "Thesaurus", function ($http, URI, Enumerable, guid, $filter, Thesaurus) {
	return {
		restrict   : 'EAC',
		templateUrl: '/app/abs/directives/forms/form-abs-permit.partial.html',
		replace    : true,
		transclude : false,
		scope      : {},
		link : function($scope, $element)
		{
			...
			$scope.options  = {
				...
				//geneticResourceType		: function () { return $http.get("/api/v2013/thesaurus/domains/20945FA8-C24C-4AF6-B3D9-367592AFDF48/terms", { cache: true }).then(function (o) { return Thesaurus.buildTree(o.data); }); },
				//geneticResourceOrigin	: function () { return $http.get("/api/v2013/thesaurus/domains/545CD54C-CFF3-41E8-A003-FDD278426A3A/terms", { cache: true }).then(function (o) { return Thesaurus.buildTree(o.data); }); },
			...
			$scope.$watch("document.gisFiles", function () { $scope.loadGisData() });
		...
		controller : ['$scope', "$q", 'IStorage', "authentication", "editFormUtility", function ($scope, $q, storage, authentication, editFormUtility) 
		{
			...
			$scope.init = function() {
				...
				if(identifier)
					promise = editFormUtility.load(identifier, "absPermit");
				else
					promise = $q.when({
						header: {
							identifier: guid(),
			...
			$scope.cleanUp = function(document) {
				...
				if (document.gisFiles && jQuery.isEmptyObject(document.gisFiles)) {
					document.gisFiles = undefined;
				}

				if (document.amendedPermits && jQuery.isEmptyObject(document.amendedPermits)) {
