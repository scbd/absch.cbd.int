...
export function securize(roleList, useSchemaRoles, checkEmailVerified) {
  return ["$location", "authentication", "appConfigService", "$filter", "$route", "realm", function ($location, authentication, appConfigService, $filter, $route, realm) {
    return authentication.getUser().then(function (user) {
      ...
      if (roles && !_.isEmpty(roles)) {
        roles = _.flatten(_.map(roles, function (role) {
          return realm.getRole(role);
        }));
      }
      if (useSchemaRoles) {
        var path = $location.path().replace("/register/", "");
        var schema;
        if (path.indexOf("/") > 0) schema = path.substr(0, path.indexOf("/")); else schema = path;
      ...
      if (!user.isAuthenticated) {
        ...
        if (!$location.search().returnUrl) $location.search({
